...
import { MessageCircle, Twitter, Linkedin, User } from 'lucide-react';
...
const FeedbackSection = () => {
  const contacts = [
    {
      platform: 'Peerlist',
      handle: 'yourprofile',
      url: 'https://peerlist.io/yourprofile',
      icon: User
    },
    {
      platform: 'Twitter',
  ...
  return (
    <div className="bg-gradient-to-br from-black to-gray-800 text-white py-20 px-4">
      <div className="max-w-2xl mx-auto text-center">
        <div className="mb-8">
          <MessageCircle className="w-12 h-12 mx-auto mb-6 text-white" />
          <h2 className="text-3xl font-bold mb-4">
            Let's Build Zapshot Together
          </h2>
          <p className="text-gray-300 text-lg leading-relaxed">
            Have feedback or want to request a new platform? 
          ...
          {contacts.map((contact) => {
            ...
            return (
              <a
                ...
                className="group text-white px-2 py-4 rounded-xl font-semibold transition-all duration-200 flex items-center gap-1 justify-center items-center"
              >
                {
                    contact.platform == 'Peerlist' ? (
                    ...
                    ) : <div className={`${contact.platform == 'Twitter' ? 'bg-black p-2' : 'p-1'}`}>
                            <IconComponent className={`${contact.platform == 'Twitter' ? 'text-2xl' : 'text-4xl'}`} /> 
                        </div>
                }
